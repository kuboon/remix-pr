Added a `type: "module"` database adapter to `remix.json`, so `remix db` can drive databases the CLI does not ship an adapter for. Previously `db.adapter.type` accepted only `sqlite`, `postgres`, and `mysql`, and third-party databases had to reimplement the command instead.
